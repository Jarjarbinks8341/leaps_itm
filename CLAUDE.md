...
- `strategy/signals.py` — Mode A (MACD bullish divergence + VIX filter), Mode B (MA20/MA50 trend-pullback + MACD convergence + IV rank), and Mode C (`deep_pullback_entry()`: price < MA5 < MA20 entry; `deep_rally_exit()`: price > MA5 > MA20 exit; `death_cross_regime()`: MA50 < MA200 — drives the adopted `dte_days_bear` DTE extension and the disabled-by-default `death_cross_filter` entry block) entry/exit signals, plus `in_earnings_blackout()`
